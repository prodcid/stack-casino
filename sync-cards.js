...
/* FRAME GAMES - Stack Garage, Stack Racing, Stack Markets. Each is a whole page of
   its own (the garage brings three.js). They're stored verbatim inside inert
...
const FRAMES = ['garage', 'racing', 'markets', 'gp', 'nrl', 'slots', 'olympus', 'egypt', 'arcade'];
...
function build(game, { css, script }) {
  ...
  const out = game.slice(0, ca + CSS_A.length) + '\n' + css + '\n' + game.slice(cb, ja + JS_A.length)
       + '\n' + script + '\n' + game.slice(jb);
  let res = out;
  for (const id of FRAMES) {
    const [A, B] = frameMarks(id), fa = res.indexOf(A), fb = res.indexOf(B);
    if (fa < 0 || fb < 0) throw new Error(`stack-casino.html is missing the stack-${id} markers`);
    res = res.slice(0, fa + A.length) + '\n' + frameText(id) + '\n' + res.slice(fb);
  }
  return res;
}
...
try {
  ...
  if (check) {
    if (next !== game) { console.log('x  Stack Cards / frame games out of sync — run: node sync-cards.js'); process.exit(1); }
    console.log('ok Stack Cards + frame games in sync');
    process.exit(0);
  }
  ...
  for (const id of FRAMES) {
    const ftmp = path.join(require('os').tmpdir(), `stack-${id}-check.js`);
    fs.writeFileSync(ftmp, frameScript(id));
    try { execFileSync(process.execPath, ['--check', ftmp], { stdio: 'pipe' }); }
    catch (e) { console.error(`x  stack-${id}.html has a syntax error:\n` + String(e.stderr || e.message)); process.exit(1); }
  }
  if (next === game) { console.log('ok Stack Cards already in sync'); return; }
  fs.writeFileSync(GAME, next);
  console.log(`ok Stack Cards + frame games synced (${(parts.css.length / 1024).toFixed(0)} KB css, ${(parts.script.length / 1024).toFixed(0)} KB js)`);
